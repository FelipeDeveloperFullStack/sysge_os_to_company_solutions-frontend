...
const RestRegister = ({ className, ...rest }) => {
    ...
    const handleSubmit = async (values, { setErrors, setStatus, setSubmitting }) => {
        if (values.password.trim() !== values.confirmPassword.trim()){
            const messageValidateError = 'As senhas precisam ser iguais! Por favor verifique e digite novamente.'
            setErrors({ password: messageValidateError, confirmPassword: messageValidateError })
        } else {
            try {
                //Loading.turnOn()
                axios
        ...
        <React.Fragment>
            <Formik
                ...
                validationSchema={Yup.object().shape({
                    ...
                    confirmPassword: Yup.string().max(255).required('Confirmação da senha obrigatório!')
                })}
